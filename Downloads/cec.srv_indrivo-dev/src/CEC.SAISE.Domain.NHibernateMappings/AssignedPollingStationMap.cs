...
namespace CEC.SAISE.Domain.NHibernateMappings
{
    public class AssignedPollingStationMap : IAutoMappingOverride<AssignedPollingStation>
    {
        public void Override(AutoMapping<AssignedPollingStation> mapping)
        {
            ...
            mapping.Map(x => x.ElectionStartTime).Nullable();
            mapping.Map(x => x.ElectionStartTime).CustomType<TimeAsTimeSpanType>().Nullable();
            mapping.Map(x => x.ElectionEndTime).CustomType<TimeAsTimeSpanType>().Nullable();
            mapping.Map(x => x.TimeDifferenceMoldova).Not.Nullable();
            mapping.Map(x => x.ActivityTimeExtendedFirstDay).Not.Nullable();
            mapping.Map(x => x.ActivityTimeExtendedSecondDay).Not.Nullable();
            mapping.Map(x => x.IsSuspended).Not.Nullable();
            mapping.Map(x => x.IsCapturingSignature).Not.Nullable();

            mapping.References(x => x.ElectionRound);
            mapping.References(x => x.AssignedCircumscription);
            mapping.References(x => x.PollingStation);
            mapping.References(x => x.ElectionDuration);
