...
function LoadOpenLayersScript(callback){
	if (!$.fn.OpenLayers){
	    ...
	    $.getScript('scripts/OpenLayers.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadMorrisScripts(callback){
	function LoadMorrisScript(){
		if(!$.fn.Morris){
			$.getScript('Scripts/morris.min.js', callback);
		}
		else {
			if (callback && typeof(callback) === "function") {
				callback();
...
//
function drawMap(lon, lat, elem, layers, zoomLevel, pollingStation) {

    var LayersArray = [];
    // Map initialization
    var map = new OpenLayers.Map(elem);
     
    map.addLayers(layers); // Add layers on map 
    var projectFrom = new OpenLayers.Projection("EPSG:4326"); // WGS 1984 projection
     
    var projectTo = map.getProjectionObject();
    
    var zoom = zoomLevel || 10; // Max zoom = 17
    map.zoomToMaxExtent();
    
    var lonlat = new OpenLayers.LonLat(lon, lat);
    map.setCenter(lonlat.transform(projectFrom, projectTo), zoom);

    var mapStyle = new OpenLayers.StyleMap({
        externalGraphic: 'Content/img/map-pin.png',
        graphicWidth: 40,
        graphicHeight: 32,
        graphicYOffset: -30,
        graphicXOffset: -8,
    ...
    var myLocation = new OpenLayers.Geometry.Point(lon, lat).transform(projectFrom, projectTo);

    // Define markers as "features" of the vector layer:
    var guestMarker = new OpenLayers.Feature.Vector(
		myLocation,
        { tooltip: 'Secția de votare № ' + pollingStation.Number }
	);

    var popup = new OpenLayers.Popup.FramedCloud("Popup",
        myLocation.getBounds().getCenterLonLat(), 
        new OpenLayers.Size(500, 200),
        'Secția de votare № <b>' + pollingStation.Number +
        '<br/>' + pollingStation.LocationDescription + '</b>',
        null,
    ...
    map.addControl(new OpenLayers.Control.MousePosition({ displayProjection: projectFrom }));

    //addOpenLayersCallback(map, myLocation, projectFrom, projectTo);
...
function addOpenLayersCallback(map, myLocation, fromProjection, toProjection) {
    ...
    OpenLayers.Control.Click = OpenLayers.Class(OpenLayers.Control, {
        ...
        trigger: function (e) {
            ...
            var fromLL = new OpenLayers.LonLat(myLocation.x, myLocation.y).transform(toProjection, fromProjection);
            // alert("Hello..." + lonlat1.lon + "  " + lonlat1.lat);

            // sendPostRoute(fromLL, lonlat1);
        }

    });
} 

function sendPostRoute(loc1, loc2) {

    if (loc2 != null && loc1 != null) {

        $.ajax({
            url: "http://openrouteservice.org/index.php?&zoom=15&pref=Fastest&lang=de", //куда шлем запрос,
            data: { start: loc1.lon + ',' + loc1.lat, end: loc2.lon + ',' + loc2.lat },
            type: "GET",
            dataType: "json",
...
$(document).ready(function () {
	...
	$('#main').css('min-height', height)
		.on('click', '.expand-link', function (e) {
			var body = $('body');
			e.preventDefault();
			var box = $(this).closest('div.box-legacy');
		...
		.on('click', '.collapse-link', function (e) {
			...
			var box = $(this).closest('div.box-legacy');
			var button = $(this).find('i');
			var content = box.find('div.box-content-legacy');
		...
		.on('click', '.close-link', function (e) {
			e.preventDefault();
			var content = $(this).closest('div.box-legacy');
			content.remove();
		});
