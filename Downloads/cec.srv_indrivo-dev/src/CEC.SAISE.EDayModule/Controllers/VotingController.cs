...
namespace CEC.SAISE.EDayModule.Controllers
{
    ...
    public class VotingController : BaseDataController
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            var aps = await _votingBll.GetAssignedPollingStationAsync(userData.AssignedPollingStation.Id);
            ...
            if (aps.IsSuspended)
            {
                return View("_PollingStationSuspended");
            }
        ...
        public async Task<ActionResult> SearchVoter(string idnp)
        {
            try
            {
                ...
                searchResult.IsCapturingSignature = aps.IsCapturingSignature;
        ...
        public async Task<ActionResult> UpdateVoter(UpdateVoterModel updateData)
        {
            try
            {
                ...
                var result = new UpdateVoterResultModel();
                if (aps.IsSuspended)
                {
                    result.IsPollingStationSuspended = aps.IsSuspended;
                }
                else
                {
                    var updateResult = await _votingBll.SaveUpdateVoterAsync(Mapper.Map<VoterUpdateData>(updateData));
                    result = Mapper.Map<UpdateVoterResultModel>(updateResult);
                    if (aps.IsCapturingSignature)
                    {
                        result.IsCapturingSignature = aps.IsCapturingSignature;

                        var voterDataSignature = await _votingBll.GetVoterDataSignature(updateData.VoterId);
                        if (voterDataSignature == null)
                        {
                            return Json(result);
                        }

                        result.Idnp = voterDataSignature.Idnp;
                        result.FirstName = voterDataSignature.FirstName;
                        result.LastName = voterDataSignature.LastName;
                    }
                }
        ...
        [HttpPost]
        public async Task<ActionResult> CaptureSignature(string idnp, HttpPostedFileBase file)
        {
            ...
            try
            {
                ...
                if (!aps.IsTurnoutEnabled)
                {
                    return new HttpStatusCodeResult(423, "Turnout is not enabled.");
                }

                // Assuming CaptureSignature method in your IVotingBll interface accepts byte[] for signature
                using (var binaryReader = new BinaryReader(file.InputStream))
                {
                    var fileData = binaryReader.ReadBytes(file.ContentLength);
                    var success = await _votingBll.CaptureSignature(idnp, fileData);

                    if (!success)
                    {
                        return new HttpStatusCodeResult(HttpStatusCode.InternalServerError, "Failed to capture signature.");
                    }
                }

                return new HttpStatusCodeResult(HttpStatusCode.OK, "Signature captured successfully.");
            }
            catch (Exception ex)
            {
                _logger.Error(ex, $"Error capturing signature for IDNP: {idnp}");
                return new HttpStatusCodeResult(HttpStatusCode.InternalServerError, "An error occurred while capturing the signature.");
            }
        }

        [HttpGet]
        public async Task<ActionResult> GetVoterDataForSignature(long voterId)
        {
            var voterDataSignature = await _votingBll.GetVoterDataSignature(voterId);
            if (voterDataSignature != null)
            {
                return Json(voterDataSignature, JsonRequestBehavior.AllowGet);
            }
            return Json("Voter not found");
        }
    }
}