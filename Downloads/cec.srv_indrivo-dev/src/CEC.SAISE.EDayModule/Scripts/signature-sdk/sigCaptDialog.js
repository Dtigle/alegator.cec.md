...
function getLocateString(string) {
	...
	var roStrings = {
		...
		"clear": "Ștergere",
		"defaultName": "Client",
		"defaultReason": "Confirmare a primirii buletinului de vot"
	};
...
class SigCaptDialog {
	...
	mapConfig(config) {
		...
		if (config.additionalData) {
			if (config.additionalData.visible != undefined) {
				this.config.additionalData.visible = config.additionalData.visible;
			}
			if (config.additionalData.fontFace) {
				this.config.additionalData.fontFace = config.additionalData.fontFace;
			}
			if (config.additionalData.fontSize) {
				this.config.additionalData.fontSize = config.additionalData.fontSize;
			}
			if (config.additionalData.left) {
				this.config.additionalData.left = config.additionalData.left;
			}
			if (config.additionalData.right) {
				this.config.additionalData.right = config.additionalData.right;
			}
			if (config.additionalData.width) {
				this.config.additionalData.width = config.additionalData.width;
			}
			if (config.additionalData.color) {
				this.config.additionalData.color = config.additionalData.color;
			}
			if (config.additionalData.offsetY) {
				this.config.additionalData.offsetY = config.additionalData.offsetY;
			}
		}
		if (config.attachTo) {
	...
	constructor(config) {
		this.config = {
			...
			additionalData: { visible: true, fontFace: "Arial", fontSize: 16, color: "black", offsetY: 20, offsetX: 30 },
	...
	**/
	async open(sigObj, who, why, where, integrityType, documentHash, osInfo, digitizerInfo, nicInfo, signatureAuthorInfo) {
		this.sigObj = sigObj;
		...
		this.signatory = signatureAuthorInfo;
	...
	createScreenImage(useColor) {
		...
		let additionalDataOffsetY = 0;
		// additional data
		const additionalDataString1 = "Declar pe propria răspundere că mă voi abține";
		const additionalDataString2 = "de la votarea multiplă";

		ctx.fillStyle = useColor ? this.config.additionalData.color : "#000000";
		ctx.font = this.config.additionalData.fontSize + "px " + this.config.additionalData.fontFace;
		let metrics = ctx.measureText(additionalDataString1);
		let actualHeight = metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent;
		additionalDataOffsetY = this.canvas.height - buttonsTop - actualHeight;
		if (this.config.additionalData.visible) {
			ctx.fillText(additionalDataString1, this.canvas.width - metrics.width - this.config.additionalData.offsetX + 20, additionalDataOffsetY - 15);
			ctx.fillText(additionalDataString2, this.canvas.width - metrics.width - this.config.additionalData.offsetX + 192, additionalDataOffsetY);
		}
		...
		if (this.signatory) {
			...
			//let fontHeight = metrics.fontBoundingBoxAscent + metrics.fontBoundingBoxDescent;
			let actualHeight = metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent;
			signatoryOffsetY = additionalDataOffsetY - (actualHeight * 2) - this.config.signatory.offsetY;
			if (this.config.signatory.visible) {
				ctx.fillText(this.signatory, this.canvas.width - metrics.width - this.config.signatory.offsetX, additionalDataOffsetY - actualHeight - this.config.signatory.offsetY - 22);
			}
		}
