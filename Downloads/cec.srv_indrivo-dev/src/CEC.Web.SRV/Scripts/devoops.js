...
//
function drawMap(lon, lat, elem, layers, zoomLevel, isLocation) {
	var LayersArray = [];
	// Map initialization
	...
	var mapStyle = new OpenLayers.StyleMap({
		externalGraphic: imageUrl + "Content/img/map-pin.png",
		graphicWidth: 40,
		graphicHeight: 32,
		graphicYOffset: -30,
		graphicXOffset: -8
	...
	if (isLocation) {
		var myLocation = new OpenLayers.Geometry.Point(lon, lat).transform(epsg4326, projectTo);
		var guestMarker = new OpenLayers.Feature.Vector(myLocation);
		var layerGuest = new OpenLayers.Layer.Vector("", {
			styleMap: mapStyle
		});
		...
		//	null, 'Longitudinea <b>' + lon + '</b>' + '<br/>' + "Latitudinea <b>" + lat + '</b>', null, false);
		//map.addPopup(popup);
	}
	...
	addOpenLayersCallback(map, epsg4326, projectTo);
...
function addOpenLayersCallback(map, fromProjection, toProjection) {
	...
	OpenLayers.Control.Click = OpenLayers.Class(OpenLayers.Control, {
		...
		trigger: function (e) {
			...
			$('#Longitude').text(lonlat1.lon);
			$('#Latitude').text(lonlat1.lat);
			
			var myLocation = new OpenLayers.Geometry.Point(lonlat1.lon, lonlat1.lat).transform(fromProjection, toProjection);
			var guestMarker = new OpenLayers.Feature.Vector(myLocation);
			var layerGuest = new OpenLayers.Layer.Vector("", {
				styleMap: mapStyle
			});
...
function MessagesMenuWidth(){
    ...
    if ($.onMenuWidthChange !== undefined && typeof ($.onMenuWidthChange) === "function") {
        $.onMenuWidthChange.call();
    }
}
//
