...
namespace CEC.SAISE.BLL.Impl
{
    public class VotingBll : IVotingBll
    {
        ...
        private async Task ReceivedBallotSupplementaryActionAsync(SystemUser user, VoterUpdateData updateData)
        {
            ...
            {
                region = _repository.LoadProxy<Region>(user.RegionId.GetValueOrDefault());
            }
            else
            {
        ...
        public Task<AssignedPollingStation> GetAssignedPollingStationAsync(long electionId, long pollingStationId)
        {
            return _repository.QueryAsync<AssignedPollingStation>(
                    z => z.FirstOrDefault(x => x.PollingStation.Id == pollingStationId && x.ElectionRound.Election.Id == electionId));
        }
        public Task<AssignedPollingStation> GetAssignedPollingStationAsync(long pollingStationId)
        {
            return _repository.QueryAsync<AssignedPollingStation>(
                    z => z.FirstOrDefault(x => x.PollingStation.Id == pollingStationId));
        }
        ...
        public async Task<bool> CaptureSignature(string idnp, byte[] signatureData)
        {
            // SQL command to update the signature
            const string updateSignatureQuery = @"
            UPDATE AVS
            SET AVS.Signature = :Signature
            FROM [dbo].[AssignedVoterStatistics] AS AVS
            JOIN [dbo].[AssignedVoter] AS AV ON AVS.AssignedVoterId = AV.AssignedVoterId
            JOIN [dbo].[Voter] AS V ON AV.VoterId = V.VoterId
            WHERE V.Idnp = :Idnp";

            bool updated = false;
            using (ISession session = _sessionFactory.OpenSession())
            {
                // Begin a transaction for update operation
                using (ITransaction transaction = session.BeginTransaction())
                {
                    // Execute the SQL update
                    int rowsAffected = session.CreateSQLQuery(updateSignatureQuery)
                        .SetParameter("Signature", signatureData, NHibernateUtil.BinaryBlob)
                        .SetInt64("Idnp", long.Parse(idnp))
                        .ExecuteUpdate();
        ...
        public async Task<VoterDataSignature> GetVoterDataSignature(long voterId)
        {
            var voterData = await _repository.QueryAsync<Voter>(z => z.FirstOrDefault(x => x.Id == voterId));
            return new VoterDataSignature()
            {
                Idnp = voterData.Idnp.ToString("D13"),
                FirstName = voterData.NameRo,
                LastName = voterData.LastNameRo
            };
        }
