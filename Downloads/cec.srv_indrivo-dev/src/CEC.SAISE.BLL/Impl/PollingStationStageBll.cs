    ...
    public class PollingStationStageBll : IPollingStationStageBll
    {
        ...
        public PageResponse<PollingStationStageEnablerDto> GetPollingStation(PageRequest pageRequest, long electionId)
        {
            ...
            var result = _repository.QueryOver<AssignedPollingStation>()
                ...
                .JoinAlias(x => x.ElectionDuration, () => ed)
                ...
                .SelectList(list => list
                    ...
                    .Select(x => x.IsSuspended).WithAlias(() => dto.IsSuspended)
                    .Select(x => x.IsCapturingSignature).WithAlias(() => dto.IsCapturingSignature)
                    .Select(x => x.ElectionStartTime).WithAlias(() => dto.ElectionStartTime)
                    .Select(x => x.ElectionEndTime).WithAlias(() => dto.ElectionEndTime)
                    .Select(x => x.TimeDifferenceMoldova).WithAlias(() => dto.TimeDifferenceMoldova)
                    .Select(x => x.ActivityTimeExtendedFirstDay).WithAlias(() => dto.ActivityTimeExtendedFirstDay)
                    .Select(x => x.ActivityTimeExtendedSecondDay).WithAlias(() => dto.ActivityTimeExtendedSecondDay)
                    .Select(() => ps.NumberPerElection).WithAlias(() => dto.PollingStation)
                    .Select(() => ed.Name).WithAlias(() => dto.ElectionDuration)
        ...
        public bool UpdatePollingStationActivity(UpdatePollingStationActivityDto updatePSActivity)
        {
            var affectedPollingStations = GetPollingStations(updatePSActivity.Ids);
            var electionDuration = _repository.Get<ElectionDuration>(updatePSActivity.ElectionDurationId);
            try
            {
                foreach (var affectedPollingStation in affectedPollingStations)
                {
                    MapPollingStationActivityChanges(affectedPollingStation, updatePSActivity, electionDuration);
                    _repository.SaveOrUpdate(affectedPollingStation);
                }
        ...
        private List<AssignedPollingStation> GetPollingStations(List<long> ids)
        {
            return _repository.Query<AssignedPollingStation>()
                .Where(ps => ids.Contains(ps.Id))
                .ToList();
        }

        public List<ElectionDurationDto> GetElectionDurations()
        {
            var electionDurationList = _repository.Query<ElectionDuration>().ToList();
            return MapElectionDurationToDto(electionDurationList);
        }

        private List<ElectionDurationDto> MapElectionDurationToDto(List<ElectionDuration> durations)
        {
            return durations.Select(d => new ElectionDurationDto
            {
                Id = d.Id,
                Name = d.Name
            }).ToList();
        ...
        private void MapPollingStationActivityChanges(
                    AssignedPollingStation affectedPollingStation,
                    UpdatePollingStationActivityDto updatePSActivity,
                    ElectionDuration electionDuration)
        {
            affectedPollingStation.ElectionStartTime = updatePSActivity.ElectionStartTime;
            affectedPollingStation.ElectionEndTime = updatePSActivity.ElectionEndTime;
            affectedPollingStation.ActivityTimeExtendedFirstDay = updatePSActivity.ActivityTimeExtendedFirstDay;
            affectedPollingStation.ActivityTimeExtendedSecondDay = updatePSActivity.ActivityTimeExtendedSecondDay;
            affectedPollingStation.TimeDifferenceMoldova = updatePSActivity.TimeDifferenceMoldova;
            affectedPollingStation.ElectionDuration = electionDuration;
        }

        public bool SuspendPollingStationActivity(List<long> ids, bool suspended)
        {
            var affectedPollingStations = GetPollingStations(ids);

                foreach (var affectedPollingStation in affectedPollingStations)
                {
                    affectedPollingStation.IsSuspended = suspended;
                    _repository.SaveOrUpdate(affectedPollingStation);
                }
                return true;
        }
        public bool ActivateCaptureSignature(List<long> ids, bool isCapturingSignature)
        {
            var affectedPollingStations = GetPollingStations(ids);
                foreach (var affectedPollingStation in affectedPollingStations)
                {
                    affectedPollingStation.IsCapturingSignature = isCapturingSignature;
                    _repository.SaveOrUpdate(affectedPollingStation);
                }
                return true;
        }
