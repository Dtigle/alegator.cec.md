...
namespace SoftTehnica.MvcReportViewer
{
    public class ReportExportResult
    {
        ...
        internal void SetParameters(ReportService.ItemParameter[] definedReportParameters, List<ReportParameter> userParameters)
        {
            if (definedReportParameters != null)
            {
                foreach (var definedReportParameter in definedReportParameters)
                {
                    ...
                    if (userParameters != null && userParameters.Any(s => s.Key == definedReportParameter.Name))
                    {
                        reportParameter.SelectedValues = userParameters.FirstOrDefault(s => s.Key == definedReportParameter.Name).Values.ToList();
                    }
