...
namespace CEC.Web.Api.Infrastructure
{
	public static class SelectListItemExtension
	{
		...
		public static IEnumerable<SelectListItem> ToSelectListUnEncrypted<T>(
			...
			Func<T, string> textFieldFunc)
		{
			return ToSelectList(source, selectedIds, useDefaultOption, "Select default option", x => valueFunc(x).ToString(), textFieldFunc);
		}
		...
		public static IEnumerable<SelectListItem> ToSelectList<T>(
			...
			Func<T, string> textFieldFunc)
		{
			return ToSelectList(source, new List<long> { selectedId }, useDefaultOption, defaultOptionText,
