...
namespace CEC.Web.SRV.EDayExport
{
    public class VoterExporter : ExportStageExecuter
    {
        ...
        private void UpdateSrvAllVoterToBeExported()
        {
            ...
            const string query = "update CEC.SRV.Domain.Person set exportedToSaise = 0";
        ...
        private void UpdatePersonExportStatus(IEnumerable<long> personIds, int exportedToSaise = 1)
        {
            ...
            {
                string query = string.Format("update CEC.SRV.Domain.Person set exportedToSaise = {1} where personId IN ({0})", string.Join(",", personIds), exportedToSaise);
                var updateQuery = Session.CreateQuery(query);
                updateQuery.ExecuteUpdate();
            }
        }
