...
{
    public abstract class ExportStageExecuter
    {
        private readonly ProgressInfo _stageProgress;
        private SaiseExporterStage _saiseExporterStage;
        ...
        public bool InterruptPending
        {
            get
            {
                _saiseExporterStage = _session.Get<SaiseExporterStage>(_saiseExporterStage.Id);
                if (_saiseExporterStage.Status == SaiseExporterStageStatus.Cancelled || _saiseExporterStage.Status == SaiseExporterStageStatus.Failed)
                {
                    return true;
                }
                else
                {
                    return _interruptPending;
                }
            }
            set
            {
                value = _interruptPending;
            }
        }

        protected ExportStageExecuter(
        ...
        private void SetStatus(SaiseExporterStageStatus status, string errorMessage = null)
        {
            ...
            if (status == SaiseExporterStageStatus.Failed)
            {
                throw new ExporterBusinessException(errorMessage);
            }
        }

        public void SetProgress(string statistics)
        {
            using (var transaction = _session.BeginTransaction())
            {
                _saiseExporterStage = _session.Get<SaiseExporterStage>(_saiseExporterStage.Id);
                _saiseExporterStage.Statistics = statistics;
                _session.Update(_saiseExporterStage);
                transaction.Commit();
            }
        }
