(function ($) {
    ...
    $.openDialog = function ($elem, _options) {
        ...
        //disable all jqgrid buttons before showing modal, so the user can't open more modals
        ...
        var window = new BootstrapDialog({
            ...
            onshown: function (dialogRef) {
                ...
                //enable all jqgrid buttons after modal was shown
    ...
    function bindToDialogForm(window, $elem, options) {
        ...
        submitForm.ajaxForm({
            ...
            beforeSubmit: function (data, form) {
                window.getModalBody().append("<div class='modal-loading'><div class='loading-text'>Vă rugăm așteptați ...</div></div>");
            ...
            complete: function (response) {
                window.getModalBody().find(".modal-loading").remove();
            }
        });
    }
    ...
    function onSuccess(window, $elem, options, html) {
        ...
        if (idx >= 0) {
            ...
            var message = html.substring(2,html.length);
            idx = message.indexOf(';');
            if (idx >= 0)   {
    ...
    //cookies for saving state of the grid
    function setCookies(grid) {
        ...
        colModel.forEach(function(model) {
            if (model.hidden === false)
                colVisible.push(model.name);
            else
                colHidden.push(model.name);
        });

        localStorage.setItem(gridId + "_visible", colVisible);
        localStorage.setItem(gridId + "_hidden", colHidden);
    }
    //**********
    ...
    $.ajaxSetup({
        error: function (xhr) {
            //enable buttons if modal request failed
            $(".ui-pg-button").removeClass("ui-state-disabled");
            //*******
    ...
    //*******
    //Set Filters to Like ("Contains") instead of Equals where possible for every grid
    ...
    $.saveGridState = function (grid) {
        var pathname = grid[0].p.pager;
        
        var rez = grid.jqGrid('getGridParam', 'colModel');

        if (rez == undefined) return;

        var sizes = new Array(rez.length);
        rez.forEach(function (value, i) {
            sizes[i] = value.width;
        });

        //store in localStorage
        localStorage.setItem("grid-saves-" + pathname, sizes.join(","));
    }

    $.loadGridState = function (grid) {
        var width = grid.width();
        var height = grid.height();

        var visibleCols = localStorage.getItem(grid.attr("id") + "_visible");
        var hiddenCols = localStorage.getItem(grid.attr("id") + "_hidden");

        if (hiddenCols) grid.jqGrid('hideCol', hiddenCols.split(","));
        if (visibleCols) grid.jqGrid('showCol', visibleCols.split(","));

        var url = grid.jqGrid('getGridParam', 'url').replace("replace-grid", "");
        url = url.replace("//", "/");
        ...
        var cfg = localStorage.getItem("grid-saves-" + pathname);
        var items = cfg ? cfg.split(/,/) : new Array();

        var rez = grid.jqGrid('getGridParam', 'colModel');

        rez.forEach(function (value, i) {
            var size = items[i];
            grid.setColWidth(i, parseInt(size), false);
        });
...
//*******
//Set Filters to Like ("Contains") instead of Equals where possible for every grid
function setFiltersForColModel(colModel) {
    colModel.forEach(function (entry) {
        if (entry.hasOwnProperty('searchoptions') && entry.searchoptions.hasOwnProperty('sopt')) {
...
//            if (entry.searchoptions.sopt[2] == 'bw') {
//                $('a.soptclass[colname=' + entry.name + ']').attr('soper', entry.searchoptions.sopt[2]);
//                $('a.soptclass[colname=' + entry.name + ']').text('^');
//            }

            if (entry.searchoptions.sopt[6] == 'cn') {
                $('a.soptclass[colname=' + entry.name + ']').attr('soper', entry.searchoptions.sopt[6]);
                $('a.soptclass[colname=' + entry.name + ']').text('~');
            }
        }
    });
